...
var IFlyIatSession = (function (window, navigator) {
    ...
    var newRecorderWorker = function (path) {
        ...
        //trows  recorderWorker.sendData(e.inputBuffer.getChannelData(0));
    ...
    var newSpeexWorker = function (path) {
        ...
        speexWorker.onmessage = function (e) {
            ...
            //trows source 编码后的数据出口 由RecorderWorker调用
    ...
    var command = (function () {
        ...
        //trows 将obj对象序列化 再反序列化。。。为了压入新的信息
        var clone = function (obj) {
            return JSON.parse(JSON.stringify(obj));
        };

        //trows socket.send(msg); 向讯飞服务器发送音频信息
        //trows 写入基础参数信息
    ...
    var newSocket = function (url) {
        ...
        return {
            ...
            "send": function (msg) {
                ...
                //trows 通过 WebSocket(向讯飞服务器发送msg) 京me从此步骤接入 生成msg信息
                $('#message').append(JSON.stringify(msg));
    ...
    var audioSender = (function () {
        ...
        var sendData = function () {
            ...
            var str = (function () {//从缓冲区获取数据
                ...
                //trows 一种通用的固定长度缓冲区类型，允许读取缓冲区中的二进制数据
            ...
            var data = {
                ...
                "audio": utils.base64encode(str)       //trows 将缓冲区获取的数据经过base64编码压入对象
            };
            //trows 正式写入编码信息
            command.audioWrite(sessionInfo.id, data);
            frameCount = 0;//trows 每6帧一次 写入完成计数清空
        };
    ...
    //trows 通过navigator.getUserMedia 获取
    var gotStream = function (stream) {
        audioStream = stream;   //trows 赋予流信息

        //trows audioCtx = window.AudioContext()
        ...
        //trows AudioContext 接口的createScriptProcessor() 方法创建一个ScriptProcessorNode
        // 用于通过JavaScript直接处理音频.  env.bufferSize系统缓冲区大小
        audioNode.scriptNode = audioCtx.createScriptProcessor(env.bufferSize, 1, 1);

        //trows 获取到系统的采样率啊   京me 通过回传流时需要此参数
    ...
    return function (setting) {
        ...
        //init
        (function () {
            //api trows html5 音频对象获取方法  如果获取过 则继续使用当前对象（navigator.getUserMedia）  否则 则使用浏览器内核提供的对象 （navigator.webkitGetUserMedia etc...  ）   京ME可从此步骤接入
            navigator.getUserMedia = navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia;
            //AudioContext接口表示由音频模块连接而成的音频处理图  京ME提供此方法支持
        ...
        recorderWorker = newRecorderWorker('xf/js/trows.recordWork.js');
        // speexWorker = newSpeexWorker(settings.speexWorkerPath);
        speexWorker = newSpeexWorker('xf/js/trows.speexWork.js');
        console.log('reset2');
